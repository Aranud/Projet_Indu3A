...
void Lidar::slotOnDataExtractedReady(QByteArray p_baDataExtracted)
{
    if(p_baDataExtracted.length() == 813)
    {
        ...
        {
            m_lstiDistance.append(p_baDataExtracted.mid(iIncrement * 2, 2).toHex().toInt(0, 16));
            m_lstiAlbedo.append(QString::number(p_baDataExtracted.at(iIncrement + 542)).toInt(0,16));
        }
